...
RvizVisualizer::RvizVisualizer(const std::string& frame_id, const std::string& topic,
  ros::NodeHandle& nh)
{
  frame_id_ = frame_id;
  topic_ = topic;
...
void RvizVisualizer::publishCube(const Eigen::Vector3d& position,
  const Eigen::Quaterniond& orientation, const Eigen::Vector3d& scale, const Colors& color,
  const std::string& ns)
{
...
void RvizVisualizer::publishLineStrip(const std::vector<Eigen::Vector3d>& points,
  const Eigen::Quaterniond& orientation, double scale, const Colors& color, const std::string& ns)
{
  visualization_msgs::Marker marker;
...
void RvizVisualizer::publishPolygon(const std::vector<Eigen::Vector3d>& points,
  const Eigen::Quaterniond& orientation, double scale, const Colors& color, const std::string& ns)
{
...
void RvizVisualizer::initMarker(visualization_msgs::Marker& marker, const std::string& ns,
  int type, int action)
{
  ...
  std::vector<geometry_msgs::Point>& points_msg)
{
  ...
  for (size_t i = 0; i < points.size(); i++)
    tf::pointEigenToMsg(points[i], points_msg[i]);
}
